...
def valores_em_reais(texto: str) -> list[tuple[str, float]]:
    valores = []
    for m in _VALOR.finditer(texto):
        inteiro, centavos = m.group(1), m.group(2) or "0"
        # "1.498" é milhar; "399.00" é o modelo escrevendo decimal à americana.
        if re.fullmatch(r"\d+\.\d{2}", inteiro):
            inteiro, centavos = inteiro.split(".")
        valores.append((m.group(0), int(inteiro.replace(".", "")) + int(centavos) / 10 ** len(centavos)))
    return valores


_KB = "\n\n".join(p.read_text(encoding="utf-8") for p in sorted((Path(__file__).parent.parent / "kb").glob("*.md")))
PRECOS = {v for _, v in valores_em_reais(_KB)}
